      ...
      <div class="with-card">
        <van-cell-group class="with-item" v-for="(item,index) in list" :key="index">
          ...
          <van-cell class="item-content" :is-link="buttonValue.edit.show" @click="buttonValue.edit.show && !buttonValue.edit.disabled ? jumpPage(item) : ''">
            ...
            <div class="content-row">
              <span>扣款金额：{{item[9] | formatMoney}}</span>
            </div>
            <div class="content-row">
        ...
        <div class="margin-top-sm" v-if="filterParams === 1">
          <van-button type="primary" size="large" @click="onAdd" v-if="buttonValue.add.show" :disabled="buttonValue.add.disabled">新增扣款单</van-button>
        </div>
      </div>
    </div>
    ...
    pageInit() {
      ...
      this.getData().then(res => {
        if (!res && this.list.length === 0) {
          ...
          index
            .getAppletButton(this.userId.UCML_UserOID, "BPO_Money_KK_InOutList")
            .then(res => {
              if (res.status) {
