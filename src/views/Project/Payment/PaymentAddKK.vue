  ...
  <div class="task">
    <div class="task-content">
      <van-cell-group :style="tabsShow ? 'padding-bottom: 280px;' : 'padding-bottom: 30px;'">
        ...
        <van-field :value="data[10] || '请选择扣款日期'" label="扣款日期" readonly required @click="showDate" />
        ...
        <van-field v-model="data[18]" label="修改日期" :disabled="true" v-if="data[18]" />
      </van-cell-group>
...
export default {
  data() {
    return {
      ...
      bpoName: "BPO_Start_KK_InOutFormService",
      projectShow: false,
      data: [],
      projectList: [],
      currProject: []
  ...
  methods: {
    ...
    onSave() {
      ...
      financial.getMemorySheetNo("KK").then(result => {
        try {
          if (result && result.status === 1) {
            ...
            xmlString += xml({
              BC_SC_Money_InOut: [
                ...
                { Sheet_Type: "KK" },
                { Approve_Flag: 0 },
                { InOut_Date: this.data[10] }, //扣款日期
                { InOut_Amt: this.data[9] }, //扣款金额
                { Operator: this.data[13] }, //经手人
                { Remark: this.data[16] }, //备注
